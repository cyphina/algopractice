...
namespace StringUtils
{
   ...
   {
      const auto IsSpace = [](char c) static
      {
         return std::isspace(c);
      };

      const auto TrimStart{std::ranges::find_if_not(Data, IsSpace)};
      // base() gives reverse iterator to underlying but it's always one element before.
      // E.g., "   WEEEE!  " end points to one past !.
      auto       ReverseData{Data | std::views::reverse};
      const auto TrimEnd{std::ranges::find_if_not(ReverseData, IsSpace).base()};
